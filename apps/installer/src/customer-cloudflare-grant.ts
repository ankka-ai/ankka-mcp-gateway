...
export type CustomerCloudflareGatewayOperation = 'source-add' | 'bigquery-add' | 'bigquery-remove' | 'upgrade' | 'rollback' |
...
function accountProbePath(operation: CustomerCloudflareGatewayOperation, workerName: string): string {
  if (operation === 'source-add' || operation === 'bigquery-add' || operation === 'bigquery-remove' || operation === 'uninstall') return '/access/ai-controls/mcp/portals';
