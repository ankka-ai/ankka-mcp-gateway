...
const OWNERSHIP_ROUTE = /^\/oauth\/relay-ticket\/(?:challenge|issue)\/(install|upgrade|rollback|source-add|bigquery-add|bigquery-remove|source-update|source-remove|management-credential|uninstall)$/u;
...
function ownershipOperation(pathname: string): CustomerCloudflareOperation | null {
  ...
  return operation === 'install' || operation === 'upgrade' || operation === 'rollback' ||
    operation === 'source-add' || operation === 'bigquery-add' || operation === 'bigquery-remove' ||
