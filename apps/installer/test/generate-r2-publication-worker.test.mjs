...
describe('release-specific R2 publication Worker generator', () => {
  ...
  it.each([1_777_000, 12_000_000])('generates a bounded signed 17-file release totaling %i object bytes', async (totalObjectBytes) => {
    ...
    try {
      ...
      expect(Buffer.byteLength(dataModule)).toBeLessThanOrEqual(16_333_332);
  ...
  it('rejects a valid signed release one byte above the raw aggregate limit before creating output', async () => {
    const input = await fixture({ totalObjectBytes: 12_000_001 });
    try {
      const plan = await readPlan(input);
      expect(plan.totalByteSize).toBe(12_000_001);
  ...
  it('retains the independent generated-module cap when permitted paths expand metadata', async () => {
    const input = await fixture({ totalObjectBytes: 12_000_000, extraAdminFiles: 395, fileNamePadding: 200 });
    try {
      const verified = await loadVerifiedR2PublicationDirectory(input.publishDirectory);
      expect(verified.plan.objectCount).toBe(402);
      expect(verified.plan.totalByteSize).toBe(12_000_000);
      ...
      expect(minimumModuleBytes).toBeGreaterThan(16_333_332);
