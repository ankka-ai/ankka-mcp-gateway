...
function containsCredentialLiteralAssignment(text) {
  ...
  for (const match of text.matchAll(quoted)) {
    // The authorization modules name fixed public token endpoints. Only these
    // exact identifier/value pairs are protocol metadata, never credentials.
    if (match[1] === 'GOOGLE_TOKEN_ENDPOINT' && match[3] === 'https://oauth2.googleapis.com/token') continue;
    if (match[1] === 'META_ADS_TOKEN_ENDPOINT' && match[3] === 'https://graph.facebook.com/v26.0/oauth/access_token') continue;
