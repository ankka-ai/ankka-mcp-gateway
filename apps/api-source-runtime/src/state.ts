...
/** One source and connection per deployment. No arguments, results, credentials or logs are stored. */
export class ApiSource extends DurableObject<Env> {
  async manage(text: string): Promise<Response> {
    try {
      ...
      const state = await this.ctx.storage.get<State>(KEY) ?? initial();
      if (command.operation === 'read') {
        return Response.json({ ...state, connection, guide: AUTHORING_GUIDE, endpoint: `${this.env.PUBLIC_ORIGIN}/mcp` });
      }
      ...
      if (command.operation === 'test') {
        ...
        return await this.ctx.storage.transaction(async (storage) => {
          const current = await storage.get<State>(KEY) ?? initial();
          ...
          await storage.put(KEY, { ...current, tested: tested.sort() });
      ...
      return await this.ctx.storage.transaction(async (storage) => {
        ...
        return Response.json({ revision: current.revision, active: current.active !== null,
          tools: current.active?.definition.tools.map((tool) => tool.name) ?? [], endpoint: `${this.env.PUBLIC_ORIGIN}/mcp` });
      });
