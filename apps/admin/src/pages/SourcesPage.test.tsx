...
describe('SourcesPage', () => {
  ...
  it.each([true, false])('leads a gateway without its management token to the one way of adding it (empty=%s)', async (empty) => {
    ...
    const api: GatewayAdminApi = {
      ...
      getTeam: vi.fn(async () => ({
        ...
        sources: [], pendingAction: null, proposedMembers: null, teams: [], proposedTeams: null,
      })),
  ...
  it('keeps the token card for a missing token only: a token it cannot read points to Settings, and a gateway that installs asks nothing', async () => {
    ...
    expect(await screen.findByRole('button', { name: 'Add connector' })).toBeEnabled()
  ...
  it('reads its connectors again when the token arrived after the dashboard loaded, instead of calling installation paused', async () => {
    ...
    render(<GatewayProvider api={api}><SourcesPage /></GatewayProvider>)
    await waitFor(() => expect(screen.getByRole('button', { name: 'Add connector' })).toBeEnabled())
  ...
  it('uses a reviewed catalog entry only as a seed for inspection and exact-tool review', async () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Add custom connector' })).toBeVisible()
    await user.click(screen.getByRole('button', { name: 'Connector library' }))
    expect(screen.getByRole('dialog', { name: 'Connector library' })).toBeVisible()
...
describe('Add BigQuery setup', () => {
  it('switches between setup dialogs and restores focus to Add connector on Escape', async () => {
    const user = userEvent.setup()
    const api = actionApi({ schemaVersion: 1, actions: [], blockingAction: null })
    ...
    expect(screen.getByRole('dialog', { name: 'Linear' })).toBeVisible()
    ...
    expect(screen.getByRole('dialog', { name: 'Add BigQuery' })).toBeVisible()
    expect(screen.getAllByRole('dialog')).toHaveLength(1)
    await waitFor(() => expect(screen.getByRole('heading', { name: 'Add BigQuery' })).toHaveFocus())
    await user.click(screen.getByRole('button', { name: 'Close BigQuery setup' }))
    await waitFor(() => expect(screen.queryByRole('dialog')).not.toBeInTheDocument())
    expect(addConnector).toHaveFocus()
  })
  ...
  it('shows the retained Google error after a fresh page load and resumes through BigQuery, even with a management token', async () => {
    ...
    render(<GatewayProvider api={api}><SourcesPage /></GatewayProvider>)
    await screen.findByRole('article', { name: `Installation of ${draft.label}` })
    expect(screen.getByText('BigQuery setup failed')).toBeVisible()
