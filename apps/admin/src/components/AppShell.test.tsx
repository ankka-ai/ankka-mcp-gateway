...
function api(overrides: Partial<GatewayAdminApi> = {}): GatewayAdminApi {
  return {
    ...
    getSourceActions: vi.fn(async () => removalRecorded), getSourceAction: vi.fn(), cancelSourceAction: vi.fn(), getSourceActionTools: vi.fn(), authorizeSource: vi.fn<GatewayAdminApi['authorizeSource']>(), chooseSourceActionTools: vi.fn(), getInstalledSourceTools: vi.fn(), updateInstalledSourceTools: vi.fn(), renameInstalledSource: vi.fn(),
    prepareRuntimeAction: vi.fn(), getRuntimeAction: vi.fn(),
    getManagementCredentialStatus: vi.fn(async () => ({ schemaVersion: 1 as const, managementCredentialConfigured: true, managementCredentialChoice: null })),
