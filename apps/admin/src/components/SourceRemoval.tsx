...
export function SourceRemoval({ source, pending, disabled, credentialConfigured, managedBigQuery, rollbackNote, onRemove, onRefresh }: {
  ...
  if (!credentialConfigured) return <p className="mt-5 text-sm text-kumo-subtle">Add a management token in <a className="underline" href="/settings">Settings</a> to remove this connector.</p>
  return <div className="mt-5 border-t border-kumo-line pt-4">
    {confirming || pending ? <>
      ...
      <p className="mt-1 text-sm text-kumo-subtle">{pending
        ? 'Removal has started. Continue to check saved progress and finish cleaning up this connector.'
        : 'Your team will lose access through this gateway. Its MCP server registration and connector access policies will be deleted from Cloudflare. Your upstream service and its data stay unchanged. You can add the connector again later.'}</p>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <Button variant="destructive" disabled={disabled} loading={removing} onClick={() => void remove()}>
          {removing ? 'Removing…' : pending ? 'Continue removal' : 'Remove connector'}
        </Button>
    ...
    </> : <Button variant="destructive" disabled={disabled} onClick={() => setConfirming(true)}>Remove connector</Button>}
