...
function fixtureApi(): GatewayAdminApi {
  return {
    ...
    getSourceAction: vi.fn(), cancelSourceAction: vi.fn(), getSourceActionTools: vi.fn(), authorizeSource: vi.fn<GatewayAdminApi['authorizeSource']>(), chooseSourceActionTools: vi.fn(), getInstalledSourceTools: vi.fn(), updateInstalledSourceTools: vi.fn(), renameInstalledSource: vi.fn(), prepareRuntimeAction: vi.fn(),
...
describe('WebMcpTools', () => {
  ...
  it('registers exactly one live batch through StrictMode, page restoration, and remounting', async () => {
    ...
    await waitFor(() => expect(registered.size).toBe(20))
